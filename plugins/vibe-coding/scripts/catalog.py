...
def refresh():
    ...
    entries = ['# Vibe Coding catalog', '', 'Choose the requested domain and operation. Each link opens its focused skill.', '', '| Skill | When to use | Workflows |', '|---|---|---|']
...
def main():
    ...
    p=sub.add_parser('search');p.add_argument('query',nargs='?',default='');p.add_argument('--mode',choices=MODE_NAMES);p.add_argument('--skill');p.add_argument('--json',action='store_true');p.add_argument('--lang',choices=LANGUAGES,default='en')
    ...
    rows = [{**r, 'title': locale['titles'][r['id']], 'category': locale['categories'][r['skill']], 'summary': locale['summaries'][r['skill']], 'example': locale['ui']['prompt'].format(skill=r['skill'], title=locale['titles'][r['id']], id=r['id'])} for r in rows]
    if args.json:print(json.dumps(rows,ensure_ascii=False,indent=2))
    else:
        for row in rows:print(f'{row["id"]} | ${row["skill"]} | {locale["modes"][row["mode"]]}\n  {row["title"]} — {row["category"]}')
