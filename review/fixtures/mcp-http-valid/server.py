...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            request = json.loads(self.rfile.read(int(self.headers.get('Content-Length', '0'))), parse_constant=reject_constant)
        except ValueError:
            return self.error(400, None, -32700, 'Parse error')
        try:
            request_id = request['id']
            if request.get('jsonrpc') != '2.0' or type(request_id) not in (int, float, str) or (type(request_id) is float and not math.isfinite(request_id)):
                return self.error(400, None, -32600, 'Invalid JSON-RPC envelope')
        ...
        if self.headers.get('Mcp-Name') != params.get('name') or not self.headers.get('Mcp-Name'):
            return self.error(400, request_id, -32020, 'HeaderMismatch')
